...
public:
  ...
  // calls the animal version of the speak, and then the cat version of speak
  void speak() const {
    Animal::speak();
    puts(" new cat speek");
  }
...
int main() {
  ...
  std::puts(" ");
  // it does not call the cat version of speak, bcs this is an animal pointer,
  // unless you declare speak as virtual
  Animal *pnt[] = {&d, &c, &p};
  for (auto p : pnt) {
    p->speak();
  }
