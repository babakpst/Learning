...
public:
    int maximumScore(vector<int>& nums, vector<int>& multipliers) {
      ...
      if (multipliers.size()==1)
        return max(multipliers[0] * nums[0],multipliers[0] * nums[nums.size()-1]);
      ...
      for (int i = m - 1; i >= 0; i--) 
      {
        for (int left = i; left >= 0; left--) 
        {
          ...
          score[left*(m+1) + i] = max(mult * nums[left] + score[i + 1+(left + 1)*(m+1)], mult * nums[right] + score[i + 1+left*(m+1)]);
        }
      }
