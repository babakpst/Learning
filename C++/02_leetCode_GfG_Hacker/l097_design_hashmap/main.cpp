...
class bucket
{
  public:
    ...
    {
      return find_if(myList.begin(), myList.end(), [&key](const pair<int,int> &x){return x.first == key;}) != myList.end();
    }
    ...
    void printBucket()
    {
      for_each(begin(myList), end(myList), [](pair<int,int> x){cout << " ("<< x.first << ", "<< x.second << ")\n";});
      cout << endl;
    }
};
...
public:
    ...
    MyHashMap(int range=769):range(range) {
      myMap.resize(range);
    }
    ...
    int get(int key) 
    {
      return  myMap[hashFunc(key)].findKey(key)? myMap[hashFunc(key)].findItem(key)->second: -1;
    }
    
    void remove(int key) {
      if (myMap[hashFunc(key)].findKey(key))
        myMap[hashFunc(key)].myList.erase(myMap[hashFunc(key)].findItem(key));
    }

    void printMap()
    {
      cout << " here is the map: " << endl;
      for_each(myMap.begin(), myMap.end(), [](auto x){ if (!x.myList.empty()) x.printBucket();});
      cout <<" --- \n";
    }
