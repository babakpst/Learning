...
// this example is from  "Discovering Modern C++" page 146
...
//T inline func_differential(F f(F), const T &x, const T &h)
T func_differential(F f(F), const T &x, const T &h)
{
  return (f(x+h)-f(x))/h;
}
//*/
/*
double func_differential(scale_sin_f f(double), const double &x, const double &h)
{
  return (f(x+h)-f(x))/h;
}
*/
...
int main(){
  ...
  //std::cout << "Functor sin("<<x<<")="<< func_differential<double, double>(scale_obj_o,x,h) <<"\n";
  //std::cout << "Functor sin("<<x<<")="<< func_differential(scale_obj_o(1.5),x,h) <<"\n";
  //std::cout << "Functor sin("<<x<<")="<< func_differential(sin_func,x,h) <<"\n";
