...
-Marking a function or a global variable as static makes it invisible to the linker once the current translation unit is compiled into an object file. Declaring a function as static prevents other files from accessing it. In other words, it is only visible to the file it was declared in; a "local" function. It prevents other translation units (.c files) from seeing the function. Keeps things clean and tidy. A function without static is extern by default (is visible to other modules).
...
public:
  ...
  static int getCount_static() {
    // cout << "length:" << length << endl;  error: static functions cannot access non-static members of the class.
    return objectCount;
  }
