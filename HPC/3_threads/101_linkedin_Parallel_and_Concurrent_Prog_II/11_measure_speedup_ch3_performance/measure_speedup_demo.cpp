...
// to run: g++ -std=c++17 -pthread measure_speedup_demo.cpp -o main
...
unsigned long long parallel_sum(unsigned int lo, unsigned int hi,
  ...
  if (depth > 3) { // base case threshold
    ...
    auto left =
        std::async(std::launch::async, parallel_sum, lo, mid, depth + 1);
