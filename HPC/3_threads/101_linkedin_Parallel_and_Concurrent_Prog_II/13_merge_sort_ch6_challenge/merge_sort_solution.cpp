...
void parallel_merge_sort(int *array, unsigned int left, unsigned int right,
  ...
  if (depth >= std::log(std::thread::hardware_concurrency())) { // base case
    sequential_merge_sort(array, left, right);
  } else {
    unsigned int mid = (left + right) / 2;
    std::thread left_thread =
        std::thread(parallel_merge_sort, array, left, mid, depth + 1);
...
void merge(int *array, unsigned int left, unsigned int mid,
           unsigned int right) {
...
int main() {
  ...
  for (int i = 0; i < NUM_EVAL_RUNS; i++) {
    std::copy(&original_array[0], &original_array[N - 1],
              sequential_result); // reset result array
  ...
  for (int i = 0; i < NUM_EVAL_RUNS; i++) {
    std::copy(&original_array[0], &original_array[N - 1],
              parallel_result); // reset result array
    auto start_time = std::chrono::high_resolution_clock::now();
