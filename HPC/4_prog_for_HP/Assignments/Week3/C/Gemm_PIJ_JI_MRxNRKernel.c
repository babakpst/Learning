...
void MyGemm( int m, int n, int k, double *A, int ldA, double *B, int ldB, double *C, int ldC )
{
  ...
  for ( int p=0; p<k; p+=KC ) {
    ...
    for ( int i=0; i<m; i+=MC ) {
      ...
      //  Gemm_JI_MRxNRKernel(ib, jb, pb, &alpha( i,p ), ldA, &beta( p,j ), ldB, &gamma( i,j ), ldC);
        Gemm_JI_MRxNRKernel(ib, n, pb, &alpha( i,p ), ldA, &beta( p,0 ), ldB, &gamma( i,0 ), ldC);
      }
    }
  }
}

void Gemm_JI_MRxNRKernel(int m, int n, int k, double *A, int ldA, double *B, int ldB, double *C, int ldC )
{
