...
//        Gemm_P_Ger( ib, jb, pb, &alpha( i,p ), ldA, &beta( p,j ), ldB, &gamma( i,j ), ldC );
//      }
      Gemm_P_Ger( ib, jb, k, &alpha( i,0 ), ldA, &beta( 0,j ), ldB, &gamma( i,j ), ldC );
    }
  }
}

void Gemm_P_Ger( int m, int n, int k, double *A, int ldA, double *B, int ldB,  double *C, int ldC )
{
  for ( int p=0; p<k; p++ )
