...
// profiler
// sudo nvprof --metrics gld_transactions,gst_transactions,shared_load_transactions,shared_store_transactions ./bin/main
...
__global__ void transpose_smem(int * in, int* out, int nx, int ny)
{
  ...
  // There are four indices to calculate:
  // 1- index to access input array in row major format.
  // 2- Index to store memory to shared memory in row major format.
  // 3- Index to load memory from shared memory in column major format.
  // 4- Index to store memory to output array in row major format.


	//1- input index
  in_index = iy * nx + ix;

  //2- 1D index calculation for shared memory - col major access
  _1d_index = threadIdx.y * blockDim.x + threadIdx.x;

  //3- col major row and col index calcuation- for shared memory
  i_row = _1d_index / blockDim.y;
  i_col = _1d_index % blockDim.y;

  //4- coordinate for transpose matrix- output transpose
