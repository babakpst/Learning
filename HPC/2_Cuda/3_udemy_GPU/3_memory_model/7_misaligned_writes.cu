...
/*
$ nvcc 7_misaligned_writes.cu -o 7_misaligned_writes
$ sudo nvprof --metrics gst_efficiency,gst_transactions ./7_misaligned_writes

>>>> output with offset 0:
Invocations                               Metric Name                        Metric Description         Min         Max         Avg
Device "NVIDIA GeForce GT 1030 (0)"
    Kernel: misaligned_write_test(float*, float*, float*, int, int)
          1                            gst_efficiency            Global Memory Store Efficiency     100.00%     100.00%     100.00%
          1                          gst_transactions                 Global Store Transactions     4194304     4194304     4194304


>>>> output with offset 11:
Invocations                               Metric Name                        Metric Description         Min         Max         Avg
Device "NVIDIA GeForce GT 1030 (0)"
    Kernel: misaligned_write_test(float*, float*, float*, int, int)
          1                            gst_efficiency            Global Memory Store Efficiency      80.00%      80.00%      80.00%
          1                          gst_transactions                 Global Store Transactions     5242878     5242878     5242878
...
__global__ void misaligned_write_test(float* a, float* b, float *c, int size, int offset)
{
  ...
  if (k < size)
    c[k] = a[gid] + b[gid]; // here global write is misalighned by offset
}
