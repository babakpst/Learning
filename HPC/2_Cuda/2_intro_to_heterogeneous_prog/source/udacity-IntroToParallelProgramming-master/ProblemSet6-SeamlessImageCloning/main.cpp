...
void preProcess(uchar4** sourceImg, size_t& numRowsSource, size_t& numColsSource, uchar4** destImg,
                uchar4** blendedImg, const std::string& source_filename,
                const std::string& dest_filename);

void postProcess(const uchar4* const blendedImg, const size_t numRowsDest, const size_t numColsDest,
                 const std::string& output_file);

void your_blend(const uchar4* const sourceImg, const size_t numRowsSource,
                const size_t numColsSource, const uchar4* const destImg, uchar4* const blendedImg);
...
int main(int argc, char** argv)
{
  ...
  switch (argc)
  {
    ...
    case 7:
      ...
      break;
    default:
      std::cerr << "Usage: ./HW6 input_source_file input_dest_filename [output_filename] "
  ...
  preProcess(&h_sourceImg, numRowsSource, numColsSource, &h_destImg, &h_blendedImg,
             input_source_file, input_dest_file);
