...
Assumptions:
- The matrix size is fixed and dimensions are multiples of 1024. 
- n iteration is 5. If you are running the kernel with different iteration, please uncommenet the 
  for loops, as instructed

Optimization results:
- On my GPU (NVIDIA GeForce GT 1030 with 3 SM) the original code takes ~850 milli seconds to run 
  and the updated kernel lasts ~22 milli seconds (38x).

- On colab (NVIDIA Tesla T4 - 40 SM) the original code takes ~78.57 milli seconds and my code lasts 
  6.53 milli seconds (12x).

Run prfiler: 
sudo nvprof --metrics gld_efficiency,gst_efficiency,branch_efficiency,sm_efficiency,achieved_occupancy,warp_execution_efficiency,inst_per_warp,stall_sync,dram_utilization ./main
...
__global__ void kernel_A(float *g_data, int dimx, int niterations) 
{
  ...
  int iy = blockIdx.y * blockDim.y + threadIdx.y; // thread id in y direction in the global grid
  int idx = iy * dimx + ix; // global index in g_data (row major)
  ...
  // {
  // To have the same computation as the original code, we need to change the branch condition. 
  // There is no divergence now. 
  // Used CUDA/intrinsic functions, faster but less accurate. The results are still within the tolerance.
  // Used // 5-fold loop unrolling
    if (tidy % 4 == 0) {
...
void launchKernel(float * d_data, int dimx, int dimy, int niterations) {
  ...
  // Creating a 2D thread block to work on the (2D) array.
  // Reducing the number of threads per block, reduces the wait time at syncthreads. There are two
  // syncthreads in the kernel, which are the hotspots of the code.
  ...
  // dimy is not need in the kernel. 
  // padding the shared memory to avoid bank conflict (but I don't see a difference in the performance).
  // creating a shared memory to bring data closer to the processors.
