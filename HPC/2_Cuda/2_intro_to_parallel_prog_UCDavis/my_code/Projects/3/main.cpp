...
int main(int argc, char **argv)
{
  ...
  switch (argc)
  {
    ...
    case 6:
      ...
      break;
    default:
      std::cerr << "Usage: ./HW3 input_file [output_filename] [reference_filename] [perPixelError] "
                   "[globalError]"
                << std::endl;
      exit(1);
  }
  ...
  your_histogram_and_prefixsum(d_luminance, d_cdf, min_logLum, max_logLum, numRows, numCols,
                               numBins);
  timer.Stop();
  ...
  checkCudaErrors(cudaMemcpy(h_luminance, d_luminance, numRows * numCols * sizeof(float),
