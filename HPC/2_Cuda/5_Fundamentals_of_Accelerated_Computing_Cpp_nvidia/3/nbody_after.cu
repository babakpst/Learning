...
// ========================
__global__
void update(Body *p, float dt, int n) {
  int idx = threadIdx.x + blockIdx.x*blockDim.x;
  int stride = blockDim.x*gridDim.x;
...
int main(const int argc, const char** argv) {
  ...
  cudaDeviceGetAttribute(&numOfSMs, cudaDevAttrMultiProcessorCount, deviceId);
    
  int threadsPerBlock = 256;
  int numBlock = 32*numOfSMs;
   ...
   */
    ...
    bodyForce<<<numBlock, threadsPerBlock>>>(p, dt, nBodies); // compute interbody forces
   ...
   */
    ...
    */
    ...
    update<<<numBlock, threadsPerBlock>>>(p, dt, nBodies);
