 ...
 public:
  ...
  bool isPalindrome(ListNode *head)
  {
    if (head != nullptr && head->next != nullptr)
    {
      ListNode *slow = head;
      ListNode *fast = head;
      // ListNode *preslow = nullptr; // This is needed if the linked list is
      // odd ListNode *head_second = nullptr; // head of the second half

      // ListNode *midNode =  nullptr; // to handle linked list with odd no. of
      // nodes.
      ...
      */
      ...
      // preslow->next = nullptr; // to disconnect the second half from the
      // first
      // head_second = slow;
      ...
      // return compareLinkedLists(head, slow);
...
int main()
{
  ...
  cout << " Is the linked list palindrome? " << std::boolalpha << obj.isPalindrome(head) << endl;
  obj.printLinkedList(head);
  return 0;
}
