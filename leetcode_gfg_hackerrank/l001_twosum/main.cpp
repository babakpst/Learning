  ...
  vector<int> twoSum(vector<int>& nums, int target)
  {
    ...
    for (vector<int>::iterator iter = nums.begin(); iter != nums.end(); iter++)
    {
      for (vector<int>::iterator iter2 = iter + 1; iter2 != nums.end(); iter2++)
      {
        ...
        {
          // std::cout << " ahhha " << std::distance(nums.begin(), iter) << " " << std::distance(nums.begin(), iter2) <<
          // std::endl; res=std::vector<int>(std::distance(nums.begin(), iter),std::distance(nums.begin(), iter2));
