...
class Solution
{
 public:
  ...
  {
    int ret = 0;
    for (int i = 0; i < target.size(); i++) ret += (target[i] - '0') * pow(10, target.size() - 1 - i);
    return ret;
  }
  ...
  string nextPass(string& old, int loc, bool sign)  // sing true means +1
  {
    ...
    sign ? (out[loc] = (out[loc] == '9' ? '0' : out[loc] + 1)) : (out[loc] = (out[loc] == '0' ? '9' : out[loc] - 1));
    return out;
  }
    ...
    while (!myq.empty())
    {
      ...
      for (int i = 0; i < target.size(); i++)
        for (auto sign : {false, true})  // here check
        {
          ...
          // cout << "1:  " << newPass << endl;
          (sign ? (newPass[i] = (newPass[i] == '9' ? '0' : newPass[i] + 1))
