...
class LogAggregator
{
 public:
  ...
  vector<int> getLogsFromMachine(int machineId)
  {
    ...
    int count = 0;
    for (auto it = begin(machine[machineId]); it != end(machine[machineId]); it++) out[count++] = *it;
    return out;
  }
  ...
  vector<int> getLogsOfService(int serviceId)
  {
    ...
    int count = 0;
    for (auto it = begin(service[serviceId]); it != end(service[serviceId]); it++) out[count++] = *it;
