 ...
 public:
  bool exist(vector<vector<char>>& board, string word)
  {
    ...
    while (!ms.empty())
    {
      ...
      {
        if (visited[loc[nextletter][j].first][loc[nextletter][j].second] >=
            ...
            ((loc[nextletter][j].first == letter[0] &&
              (loc[nextletter][j].second == letter[1] + 1 || loc[nextletter][j].second == letter[1] - 1)) ||
             (loc[nextletter][j].second == letter[1] &&
              (loc[nextletter][j].first == letter[0] + 1 || loc[nextletter][j].first == letter[0] - 1))))
        {
...
class Solution
{
 ...
 public:
  ...
  bool backtracking(int row, int col, string word, int ind)
  {
    ...
    if (row < 0 || row == rows || col < 0 || col == cols || visited[row][col] != word[ind]) return false;
