 ...
 public:
  vector<int> spiralOrder(vector<vector<int>>& matrix)
  {
    ...
    while (counter < m * n)
    {
      ...
      cout << rowBoundaryTop << " " << rowBoundaryBot << " -- " << colBoundaryLeft << " " << colBoundaryRight << " ("
           << i << ", " << j << ") >> cond: " << cond << endl;
    }
    return out;
  }
...
class Solution
{
 public:
  vector<int> spiralOrder(vector<vector<int>>& matrix)
  {
    ...
    while (counter < m * n)
    {
      for (int col = colBoundaryLeft; col <= colBoundaryRight; ++col) out[counter++] = matrix[rowBoundaryTop][col];

      for (int row = rowBoundaryTop + 1; row <= rowBoundaryBot; ++row) out[counter++] = matrix[row][colBoundaryRight];
      ...
      if (colBoundaryLeft != colBoundaryRight)
        for (int row = rowBoundaryBot - 1; row > rowBoundaryTop; --row) out[counter++] = matrix[row][colBoundaryLeft];
      ++rowBoundaryTop;
      --rowBoundaryBot;
      ++colBoundaryLeft;
      --colBoundaryRight;
